    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0 }}
          className={cn(
            "absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-4 rounded-lg bg-black px-4 py-8 shadow-lg text-white",
            className,
          )}
        >
          {children}
  ...
  return (
    ...
    >
      <img
        ...
        height={30}
      />
      <span className="font-medium text-white">ADHDapt</span>
    </a>
  );
};
...
export const NavbarButton = ({
  ...
  const baseStyles =
    "px-4 py-2 rounded-md bg-black text-white text-sm font-bold relative cursor-pointer hover:-translate-y-0.5 transition duration-200 inline-block text-center";
